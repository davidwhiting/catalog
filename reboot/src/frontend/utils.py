...
async def init_original(q: Q) -> None:
    q.page['meta'] = dmfe.meta_card
    q.page['sidebar'] = dmfe.return_sidebar(q)
    q.page['header'] = dmfe.header_card
...
async def serve_original(q: Q):
    # Run only once per client connection.
    if not q.client.initialized:
        q.client.cards = set()
        await init_original(q)
        q.client.initialized = True
